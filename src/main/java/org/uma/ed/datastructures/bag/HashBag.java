...
import org.uma.ed.datastructures.hashtable.HashPrimes;
...
public class HashBag<T> extends AbstractBag<T> implements Bag<T> {
  ...
  public void delete(T element) {
    Pair<T> pairFound = this.hashTable.search( Pair.withElement(element) );

    if (pairFound != null) {
      int oldAmount = pairFound.occurrences();
      if (oldAmount == 1) {
        hashTable.delete( Pair.withElement(element) );
      } else {
        hashTable.delete( Pair.withElement(element) );
        hashTable.insert( Pair.of(element, oldAmount - 1));
      }
    }
  }
  ...
  public void insert(T element) {
    this.hashTable.insert( Pair.of(element, occurrences(element) + 1) );
  }
  ...
  public int occurrences(T element) {
    Pair<T> elementSearched = this.hashTable.search( Pair.withElement(element) );
    return elementSearched == null ? 0 : elementSearched.occurrences;
  }
  ...
  private final class HashBagIterator implements Iterator<T> {
    ...
    public T next() {
      ...
      T elementToReturn = element;
      occurrences--;
      if (occurrences == 0 ) advance();
