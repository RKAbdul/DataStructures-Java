...
public class SortedLinkedSet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  public void insert(T element) {
    ...
    if (!elementFinder.found) {
      Node<T> newNode = new Node<>(element, elementFinder.current);

      if (elementFinder.previous == null) {
        first = newNode;
      } else {
        elementFinder.previous.next = newNode;
      }
  ...
  public void delete(T element) {
    ...
    if (elementFinder.found) {
      if (elementFinder.previous == null) {
        first = elementFinder.current.next;
      } else {
        elementFinder.previous.next = elementFinder.current.next;
      }
      size--;
    }
  }
  ...
  public static <T> SortedLinkedSet<T> union(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    while(elem1 != null || elem2 != null)
    {
      ...
      {
        int cmp = comparator.compare(elem1, elem2);
