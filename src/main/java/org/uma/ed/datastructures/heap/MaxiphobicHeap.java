...
public class MaxiphobicHeap<T> implements Heap<T> {
  ...
  public void insert(T element) {
    Node<T> auxiliaryNode = new Node<>(element);
    root = merge(root, auxiliaryNode);
  }
  ...
  private Node<T> merge(Node<T> node1, Node<T> node2) {
    ...
    if (comparator.compare(node1.element, node2.element) > 0) {
      MaxiphobicHeap.Node<T> auxiliarNode = node1;
      node1 = node2;
      node2 = auxiliarNode;
    }
    ...
    node1.weight = weight(tree2) + weight(tree3) + 1;

    if (weight(node1.right) > weight(node1.left)) {
      MaxiphobicHeap.Node<T> auxiliarNode = node1.right;
      node1.right = node1.left;
      node1.left = auxiliarNode;
    }
  ...
  public void deleteMinimum() {
    if (root == null) throw new EmptyHeapException("EMPTY HEAP");
